...
void ejecutarNodoMaestro(int num_nodos){
    ...
    while (!procesamiento_finalizado || lotes_completados<total_lotes){
        ...
        if (tiempo_desde_ultimo_reporte.count() >= INTERVALO_REPORTE * SEGUNDOS_POR_MINUTO){
            imprimirEstadisticas(estadisticas_globales, MAESTRO); 
        ...
        if (tiempo_desde_ultimo_balanceo.count() >= INTERVALO_CHEQUEO_BALANCEO){
            balanceoCarga(rendimiento_nodos, cola_trabajo, num_nodos);
        ...
        if (flag) {
            ...
            switch (tag) {
                ...
                case TAG_RESULTADO_FINAL: {
                    ...
                    for (int i = 1; i < num_nodos; i++) {
                        if (rendimiento_nodos[i].lotes_completados < rendimiento_nodos[i].lotes_asignados) {
                            todos_terminados = false;
                            break;
                        }
                    }
                    
                    if (todos_terminados && cola_trabajo.empty()) {
